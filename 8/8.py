...
import warnings
import os
os.environ["PATH"] += os.pathsep + 'd:/1POIT/3/OIAD/Graphviz/bin/'
...
tree.fit(X_train, y_train)
print("С предварительной обрезкой:")
print("Правильность на обучающем наборе: {:.3f}".format(tree.score(X_train, y_train)))
print("Правильность на тестовом наборе: {:.3f}".format(tree.score(X_test, y_test)))
...
graph = pydotplus.graph_from_dot_data(dot_data)
graph.write_png("my_tree.png")  # сохранение изображения в файл
Image(filename="my_tree.png")  # отображение изображения
...
#взаимосвязь признаков и меток класса не монотонна
...
import pandas as pd
ram_prices = pd.read_csv("d:/1POIT/3/OIAD/1/8/ram_price.csv")

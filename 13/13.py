...
# устанавливаем параметры форматирования для вывода массивов
...
print(f"""\nФорма преобразованного массива: {X_train_scaled.shape}
...
# Чтобы применить SVM к масштабированным данным, мы должны преобразовать еще тестовый набор.
# Это снова делается с помощью вызова метода transform, на этот раз для X_test:
## метод transform всегда вычитает минимальное значение, вычисленное для обучающего набора, и делит на ширину диапазона, вычисленную также для обучающего набора
...
print(f"\nПравильность на тестовом наборе: {svm.score(X_test, y_test):.2f}")
...
## можно легко заменить один алгоритм предварительной обработки на другой, сменив имя используемого класса, поскольку все классы предварительной обработки имеют один и тот же интерфейс, состоящий из методов fit и transform:
from sklearn.preprocessing import StandardScaler
